...
Model families include supervised ImageNet-pretrained ResNet-18, EfficientNet-B0, ViT-Tiny, ConvNeXt-Tiny, and SimCLR-pretrained ResNet-18. Downstream evaluation uses default threshold metrics plus validation-selected thresholds optimized for balanced accuracy subject to minimum sensitivity. Calibration is measured with Brier score, ECE, and NLL; Platt scaling and temperature scaling are fit on validation predictions and evaluated on test predictions.
...
- SimCLR only so far; BYOL/DINO remain planned extensions.
